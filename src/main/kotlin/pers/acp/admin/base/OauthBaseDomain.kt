...
abstract class OauthBaseDomain(protected val userRepository: UserRepository) : BaseDomain() {
    ...
    protected fun isSuper(user: User): Boolean =
        user.roleSet.map { it.code }.toList().contains(RoleCode.SUPER) ||
                (user.roleSet.isNotEmpty() && user.roleSet.map { it.levels }.toIntArray().minOrNull() == 0)
    ...
    protected fun validateModifyRoleSet(
        userInfo: User,
        appId: String,
        oldRoleSet: MutableSet<Role>,
        newRoleSet: MutableSet<Role>
    ): Boolean =
        if (!isSuper(userInfo)) {
    ...
    @Throws(ServerException::class)
    protected fun validateModifyUserSet(
        userInfo: User,
        oldUserSet: MutableSet<User>,
        newUserSet: MutableSet<User>
    ): Boolean =
        if (!isSuper(userInfo)) {
    ...
    protected fun getAllOrgList(
        organizationRepository: OrganizationRepository,
        organizationList: MutableList<Organization>
     ...
     * 获取指定机构集合的所有子机构
     */
    private fun getChildrenOrgList(
        organizationRepository: OrganizationRepository,
        organizationList: MutableList<Organization>
    ): MutableList<Organization> =
