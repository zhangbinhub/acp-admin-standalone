...
constructor(
    ...
    @Throws(ServerException::class)
    fun currMenuList(@ApiIgnore user: OAuth2Authentication): ResponseEntity<List<Menu>> =
    ...
    @Throws(ServerException::class)
    fun currModuleFuncList(@ApiIgnore user: OAuth2Authentication): ResponseEntity<List<ModuleFunc>> =
    ...
    fun addMenu(@ApiIgnore user: OAuth2Authentication, @RequestBody @Valid menuPo: MenuPo): ResponseEntity<Menu> =
    ...
    @Throws(ServerException::class)
    fun updateMenu(@ApiIgnore user: OAuth2Authentication, @RequestBody @Valid menuPo: MenuPo): ResponseEntity<Menu> {
    ...
    @ApiResponses(
        ApiResponse(
            code = 400,
            message = "参数校验不通过；模块功能编码非法，请重新输入；没有权限做此操作；ID不能为空；找不到信息；",
    ...
    @Throws(ServerException::class)
    fun moduleFuncInfo(
        @ApiParam(value = "模块功能id", required = true)
